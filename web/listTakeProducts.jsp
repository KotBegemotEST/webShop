            ...
            <ul>
                <c:forEach var="purchase" items="${takeProducts}"><!-- конструкция списка, где каждый объект списка вставляется в переменную $(listProducts)-->
                    <li> Товар <b>"${purchase.product.name}"</b>по цене ${purchase.product.price} euro   - купил <b>
                         ...
                         <a href="returnProduct?purchaseId=${purchase.id}">     Пополнить товар на складе</a></li>
                       <!--- <input type="hidden" value="${purchase.id}" name="returnProductId">--->    <!---   "hidden" - для скрытия поляБпередающего в обработку libHistory.id--->
                       <!--- <button type="submit">Пополнить товар на складе</button>--->
                    </li>  
                </c:forEach> 
            </ul>
        </form>
        <br><p><a href="/webShop">Главная</a></p><br><br>
    </body>
</html>
